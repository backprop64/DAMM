...
# ============================ MODIFICATIONS ===================================
# this piece of code has been modified in the following ways:
# - annotation format follows COCO standard 
# - utils for sampling images from videos 
# - 
# ==============================================================================
...
def draw_bbox(image_urls, callbackId):  # pylint: disable=invalid-name
  ...
  js = Javascript('''
              async function load_image(imgs, callbackId) {
                  ...
                  crosshair_h.style.borderColor = "hsl(120, 50%, 70%)";
                  ...
                  crosshair_v.style.borderColor = "hsl(120, 50%, 70%)";
...
def rescale_and_convert_bboxes(bboxes, image_path):
...
def create_metadata(train_images,annotated_boxes):
  mini_metadata = []
  for train_image,box in zip(train_images,annotated_boxes):
    coco_dict = rescale_and_convert_bboxes(box, train_image)
    mini_metadata.append(coco_dict)
  return mini_metadata

def get_images_to_annotate(image_folder):
  ...
  if os.path.exists(image_folder):
      ...
      for filename in os.listdir(image_folder):
          # Check if the file has a common image file extension (you can customize this list)
  ...
  if os.path.exists(metadata_path):
    ...
    images2annotate = [x for x in image_paths if x.split(os.sep)[-1] not in annotated_images]
  else:
    images2annotate = image_paths

  return images2annotate

def update_dataset_metadata(image_folder,new_annotations):
  metadata_path = os.path.join(image_folder,'metadata.json')
  for ann in new_annotations:
    ann["file_name" ] = os.path.join(ann["file_name"].split(os.sep)[-2],ann["file_name"].split(os.sep)[-1])
  if os.path.exists(metadata_path):
    annotations = json.load(open(metadata_path))['annotations']
    annotated_images = [x["file_name"].split(os.sep)[-1] for x in annotations]
    new_annotations = [x for x in new_annotations if x["file_name"].split(os.sep)[-1] not in annotated_images]
